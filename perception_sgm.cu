...
// win the argmin, small enough that 8-path accumulation stays well within int.
#define SGM_INF_COST 30000

typedef unsigned short cost_t;   // per-(pixel,disparity) data term

// ---------------------------------------------------------------------------
// Data term 1: windowed SAD cost volume C[(r*W+c)*D + d].
// Same matching cost as the local SAD baseline so the comparison isolates the
// effect of path aggregation. A 5x5 8-bit window SAD <= 6375 fits in uint16.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Data term 2: Census transform + Hamming-distance cost.
// The census transform encodes each pixel as a bit string recording whether
// each window neighbour is brighter than the centre. The matching cost is the
// Hamming distance between the left and right strings. Because it depends only
// on local intensity *ordering*, census is robust to gain/exposure differences
// between the two cameras (where raw SAD fails). A (2R+1)^2-1 window fits in 64
// bits for R<=3. Hamming distance is in [0,(2R+1)^2-1].
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Per-block scanline geometry for an arbitrary path direction (dir_r, dir_c)
// with each component in {-1,0,1} and not both zero. One block aggregates one
// scanline. Returns the scanline's start pixel (r0,c0) and length (steps).
//
//  - horizontal (dir_r=0): one block per row     -> H scanlines
//  - vertical   (dir_c=0): one block per column  -> W scanlines
//  - diagonal:  scanlines enter from two borders -> W + H - 1 scanlines
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Path aggregation along (dir_r, dir_c). One block per scanline; blockDim.x =
// next_pow2(D) so the min-over-disparity reduction is a clean tree. The block
// walks its scanline sequentially (the SGM recurrence is a serial dependency
// along the path), while the independent scanlines run in parallel across
// blocks. The accumulator S is summed over all path directions.
//
//   L(p,d) = C(p,d) + min( L(p-r,d), L(p-r,d±1)+P1, min_k L(p-r,k)+P2 )
//                   - min_k L(p-r,k)
// ---------------------------------------------------------------------------
...
// ===========================================================================
// Distributed SGM (MPI): row-decomposed across ranks.
//
// With a 1-D row decomposition the four axis-aligned paths split cleanly:
//   * Horizontal (L->R, R->L): each scanline is a row owned entirely by one
//     rank, so these are computed locally with no communication.
//   * Vertical (T->B, B->T): each scanline is a column that spans the whole
//     image height and therefore crosses rank boundaries. To preserve the
//     global SGM optimum, ranks pass the per-column aggregated cost at their
//     boundary row (the "frontier", W*D ints) to the neighbouring rank, which
//     resumes the recurrence. This serializes the vertical sweep across ranks
//     (a pipeline) while the horizontal sweep stays fully parallel --- which is
//     exactly why SGM is harder to distribute than local SAD.
//
// API: sgm_dist_begin builds the cost volume and runs the horizontal paths;
// sgm_dist_vert runs one vertical direction given an incoming frontier and
// returns the outgoing one; sgm_dist_finish does the WTA + sub-pixel argmin.
// The driver (main_mpi) handles the MPI_Send/Recv of the frontier buffers.
// ===========================================================================

// Vertical aggregation over a rank's owned rows, with frontier I/O. One block
// per column. `frontier_in` (or null at the global border) seeds the previous
// row; the last owned row's costs are written to `frontier_out`.
...
void* sgm_dist_begin(const Image& slab_l, const Image& slab_r,
  ...
  // Horizontal paths (local to each rank). Aggregating halo rows too is
  // harmless because the gather only collects owned rows.
...
// Run one vertical direction (dir = +1 top->bottom, -1 bottom->top). `fin_host`
// is the incoming frontier from the upstream neighbour (null at the border);
// the outgoing frontier is written to `fout_host`.
